#include "gaussfilterinputwidget.h"

GaussFilterInputWidget::GaussFilterInputWidget(ImageService *service, ImageEditor *editor)
{
    _service = service;
    _editor = editor;
...
GaussFilterInputWidget::~GaussFilterInputWidget()
{
...
void GaussFilterInputWidget::on_execute_button_clicked()
{
    close();
    _editor->update_image_view();
}

void GaussFilterInputWidget::on_cancel_button_clicked()
{
    close();
}
