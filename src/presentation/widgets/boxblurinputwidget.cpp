#include "boxblurinputwidget.h"

BoxBlurInputWidget::BoxBlurInputWidget(ImageService *service, ImageEditor *editor)
{
    _service = service;
    _editor = editor;

    auto main_layout = new QVBoxLayout();
    auto layout1 = new QHBoxLayout();
    auto layout2 = new QHBoxLayout();
    auto layout3 = new QHBoxLayout();
    auto layout4 = new QHBoxLayout();

    main_layout->addLayout(layout1);
    main_layout->addLayout(layout2);
    main_layout->addLayout(layout3);
    main_layout->addLayout(layout4);

    _line_edit_r = new QLineEdit();
    _execute_button = new QPushButton("Execute");
    _cancel_button = new QPushButton("Cancel");

    layout3->addWidget(new QLabel("Kernel radius: "));
    layout3->addWidget(_line_edit_r);
    layout4->addWidget(_execute_button);
    layout4->addWidget(_cancel_button);

    setLayout(main_layout);

    connect(_execute_button, SIGNAL(clicked()), this, SLOT(on_execute_button_clicked()));
    connect(_cancel_button, SIGNAL(clicked()), this, SLOT(on_cancel_button_clicked()));
}

BoxBlurInputWidget::~BoxBlurInputWidget()
{
    delete _line_edit_r;
    delete _execute_button;
    delete _cancel_button;
}

void BoxBlurInputWidget::on_execute_button_clicked()
{
    close();
    _editor->update_image_view();
}

void BoxBlurInputWidget::on_cancel_button_clicked()
{
    close();
}
